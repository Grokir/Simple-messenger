...
//        String decodePassword = CryptoService.sendPOST("password", userRequest.getPassword(), true);
    ...
    public ResponseEntity<JSONObject> create(@RequestBody UserRequest userRequest){
        JSONObject jsonResponse = new JSONObject();

        if(userService.existUserWithLogin(userRequest.getLogin())){
            jsonResponse.put("status", "user exist");
            return new ResponseEntity<>(jsonResponse, HttpStatus.CONFLICT);
        }
        String userId = "";
        if(userService.create(userRequest)){
            userId = userService.findUserByLogin(userRequest.getLogin()).getId().toString();
        }
        jsonResponse.put("status", "OK");
        jsonResponse.put("userId", userId);
        jsonResponse.put("login", userRequest.getLogin());
//        String decodePassword = CryptoService.sendPOST("password", userRequest.getPassword(), true);
