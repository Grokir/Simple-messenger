...
public class MsgService implements BaseService<MsgRequest, MsgResponse>{
    ...
    public MsgResponse getReferenceById(String id) {
        Msg msg = msgRepository.findMsgById(UUID.fromString(id)).orElseThrow(() -> new RuntimeException("msg with id " + id + "not found!"));
        MsgResponse resp = null;
        if (!msg.getWasDelete()){
            resp = msgFacade.toResponse(msg);
        }
        return resp;
    }
...
//                """, in);
    ...
    public JSONArray showAllMessages(String chatId){
...
//            msg.setMsgText(CryptoService.sendPOST("msgText", msg.getMsgText(), false));
        ...
        if (!resList.isEmpty())
            Collections.sort(resList, new Comparator<MsgResponse>() {
                public int compare(MsgResponse o1, MsgResponse o2) {
                    return o1.getDateDispatch().compareTo(o2.getDateDispatch());
                }
            });
